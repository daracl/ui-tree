...
export class Tree {
  ...
  public addNode(nodeItem: any[] | any, parentId?: any, options?: any) {
    ...
    parentId = isUndefined(parentId) ? rootNodeId : parentId;
  ...
  public createNode(nodeInfo: any) {
    ...
    nodeInfo[this.options.itemKey.pid] =
      nodeInfo.pid ??
      nodeInfo[this.options.itemKey.pid] ??
      (this.config.selectedNode ? this.config.selectedNode.id : this.config.rootNode.id);
  ...
  private render(id: any) {
    ...
    if (id === this.config.rootNode.id && !this.isInit) {
      ...
      const liElement = this.treeNodeRenderer.createNodeElement(
        renderNode,
        this.options.enableRootNode ? 'display:inline' : 'display:none',
        true,
        true,
      );
   ...
   *
   * @public
   * @param {TreeNode} changeNodeInfo
   */
  public changeNodeInfo = (changeNodeInfo: TreeNode) => {
    const changeElement = this.rootElement.querySelector(`[data-dt-id="${changeNodeInfo.id}"]`);

    if (changeElement) {
      const spanEle = changeElement?.querySelector('.dt-node-title > span');

      if (spanEle) {
        spanEle.textContent = changeNodeInfo.text;
      }

      // 아이콘 활성화 일경우 아이콘 변경.
      if (this.options.enableIcon) {
        const iconElement = changeElement.querySelector('i.dt-icon');
        const icon = nodeUtils.getIcon(changeNodeInfo);
      ...
      if (nodeUtils.isFolder(changeNodeInfo) && changeElement.querySelector('.dt-expander.dt-visible') == null) {
        // 폴더가 아닐 경우 폴더로 변경.
        addClass(changeElement.querySelector('.dt-expander'), 'dt-visible');
      } else if (!nodeUtils.isFolder(changeNodeInfo) && changeElement.querySelector('.dt-expander.dt-visible')) {
        // 폴더일 경우 일반 노드로 변경.
        removeClass(changeElement.querySelector('.dt-expander'), 'dt-visible');
      }
    }
  };
  ...
  public getNodeList = (id: string | number, options: any): TreeNode[] => {
    ...
    const exportkeys = ['orgin', 'id', 'pid', 'text', 'depth'];

    function pickKeys(node: TreeNode) {
      // keys 옵션 없으면 전체 반환
