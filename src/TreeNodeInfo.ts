...
export class TreeNodeInfo implements TreeNode {
  ...
  constructor(item: any, parentId: any, tree: Tree) {
    ...
    this.id = item[itemKey.id];
    this.pid = parentId;
    this.text = item[itemKey.text];
    this.url = item.url;
    this.checkState = item.state?.checked === true ? 1 : 2;
    this.target = item.target;
    this._cud = item['_cud'] ?? '';
    this.icon = item[itemKey.icon];
    this.depth = cfg.allNode[this.pid] ? cfg.allNode[this.pid].depth + 1 : 0;
    this.orgin = item;
    this.renderChildLength = 0;

    this.stateFolder = item.state?.folder === true;

    this.isOpen = cfg.isRequest
      ? false
      : item.state?.opened === true || opts.openDepth == -1 || opts.openDepth >= this.depth;

    cfg.selectedNode = item.state?.selected === true ? item : cfg.selectedNode;
  }
